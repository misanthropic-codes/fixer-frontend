...
import { ChevronLeft, ShoppingCart, ShieldCheck, Hammer, Info, CheckCircle2 } from 'lucide-react';
...
export default async function PartDetailPage({ params }: { params: Promise<{ sku: string }> }) {
  ...
  try {
    const res = await fetch(`${apiUrl}/spare-parts/${sku}`, { cache: 'no-store' });
    if (res.ok) {
      part = await res.json();
    }
  } catch (err) {
    ...
    return (
      <>
        ...
        <div className="pt-32 pb-20 text-center">
          ...
          <Link href="/spare-parts" className="text-primary font-bold mt-4 inline-block">Back to Catalog</Link>
        </div>
        <Footer />
      </>
    );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white pb-24 md:pb-0">
      ...
      <main className="flex-1 pt-14 md:pt-28 container mx-auto px-4 max-w-6xl">
        <Link href="/spare-parts" className="inline-flex items-center gap-2 text-xs font-black uppercase text-zinc-400 hover:text-primary transition-colors mb-6">
            ...
            {part.imageUrls?.length > 1 && (
               <div className="flex gap-3 overflow-x-auto pb-2">
                  {part.imageUrls.map((url: string, i: number) => (
                    <div key={i} className="relative w-20 h-20 rounded-xl overflow-hidden border border-zinc-200 flex-shrink-0 cursor-pointer hover:border-primary transition-colors">
                       <Image src={url} alt={`${part.name}-${i}`} fill className="object-cover" />
                    </div>
                  ))}
               </div>
            )}
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2 mb-4">
              <span className={cn(
                ...
                part.partType?.type === 'OEM' ? "bg-blue-100 text-blue-700" : "bg-teal-100 text-teal-700"
              )}>
              ...
              <span className="flex items-center gap-1 text-[10px] uppercase font-black px-3 py-1 rounded-full bg-zinc-100 text-zinc-600">
                 {getApplianceIcon(part.applianceTypeSlug === 'refrigerator' ? 'Refrigerator' : 'WashingMachine', "w-3 h-3")}
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
               <div className="p-4 rounded-2xl bg-zinc-50 border border-zinc-100">
                  ...
                  <p className="text-sm font-black text-zinc-900">{part.warrantyMonths || 0} Months</p>
               </div>
               <div className="p-4 rounded-2xl bg-zinc-50 border border-zinc-100">
                  <div className="flex items-center gap-2 text-zinc-400 mb-1">
                     <Hammer className="w-4 h-4" />
                     <span className="text-[10px] font-black uppercase tracking-widest">Installation</span>
                  </div>
                  <p className="text-sm font-black text-zinc-900">{part.installationDifficulty?.type || 'Standard'}</p>
               </div>
            </div>

            <div className="space-y-6">
               <section>
                  <h3 className="text-[11px] font-black uppercase text-zinc-400 tracking-[0.2em] mb-3">Compatible Models</h3>
                  <div className="flex flex-col gap-2">
                     {part.isUniversal ? (
                        <div className="flex items-center gap-2 p-3 bg-teal-50 text-teal-700 rounded-xl">
                           <CheckCircle2 className="w-4 h-4" />
                           <p className="text-xs font-bold leading-tight">Universal fit for most {part.applianceTypeSlug} brands.</p>
                        </div>
                     ) : (
                        part.compatibleModels?.map((m: any, i: number) => (
                           <div key={i} className="flex items-center justify-between p-3 bg-zinc-50 rounded-xl border border-zinc-100">
                              <span className="text-xs font-black text-zinc-800">{m.displayName}</span>
                              <span className="text-[10px] font-black text-zinc-400 uppercase">{m.modelNumber}</span>
                           </div>
                        ))
                     )}
                     {!part.isUniversal && (!part.compatibleModels || part.compatibleModels.length === 0) && (
                        <div className="flex items-center gap-2 p-3 bg-zinc-50 text-zinc-500 rounded-xl italic">
                           <Info className="w-4 h-4" />
                           <p className="text-xs">Compatible with all {part.brandSlug} {part.applianceTypeSlug}s.</p>
                        </div>
                     )}
                  </div>
               </section>
               ...
               <section>
                  <h3 className="text-[11px] font-black uppercase text-zinc-400 tracking-[0.2em] mb-3">Description</h3>
